...
def get_llm() -> ChatGoogleGenerativeAI:
    # google_api_key is passed explicitly (not just via env var) and
    # transport is pinned to "rest" because on some hosts (e.g. Streamlit
    # Community Cloud) the default gRPC transport ignores the API key and
    # falls back to Application Default Credentials, causing a 401
    # UNAUTHENTICATED / ACCESS_TOKEN_TYPE_UNSUPPORTED error.
...
def build_vector_store(chunks: List[str]):
    ...
    # Same fix as get_llm(): pass the key explicitly and force REST
    # transport so the embeddings client doesn't try to fall back to
    # Application Default Credentials and raise UNAUTHENTICATED.
